import React from 'react'

export default function TimeLeftIcon() {
  return (
    <svg className='fill-current' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 33 33'>
      <defs>
        <style>{`.cls-1{fill:none;stroke:current;stroke-miterlimit:10;}.cls-2{fill:current;}`}</style>
      </defs>
      <g id='Layer_2' dataname='Layer 2'>
        <g id='Layer_2-2' dataname='Layer 2'>
          <circle stroke='black' className='cls-1' cx='16.5' cy='16.5' r='16' />
          <path fill='current' className='cls-2' d='M16.5,3.7V16.5H3.7A12.8,12.8,0,1,0,16.5,3.7Z' />
        </g>
      </g>
    </svg>
  )
}
